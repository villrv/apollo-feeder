...
def generate_qr_code(poodle_scale: int = 4, qr_pix_size: int = 15):
    ...
    poodle_scale = int(poodle_scale)
    qr_pix_size = int(qr_pix_size)

    # Generate the QR code
    qr = qrcode.QRCode(
        version=1,
        error_correction=qrcode.constants.ERROR_CORRECT_H,
        box_size=qr_pix_size,
        border=1,
    )
    qr.add_data(url)
    qr.make(fit=True)
    ...
    poodle = poodle.resize(((2 * poodle_scale + 1) * qr_pix_size,) * 2)

    pos = (
        (img_qr.size[0] - poodle.size[0]) // 2,
        (img_qr.size[1] - poodle.size[1]) // 2,
    )
